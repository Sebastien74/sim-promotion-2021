...
class BrowserDetection
{
    ...
    protected function matchUAAgainstKey($key)
    {
        ...
        $keyFormat = strtolower($key);

        if (false === isset($this->cache[$keyFormat])) {
            // change the keys to lower case
            $_rules = array_change_key_case($this->getRules());
            if (false === empty($_rules[$keyFormat])) {
                $this->cache[$keyFormat] = $this->match($_rules[$keyFormat]);
            }
            if (false === isset($this->cache[$keyFormat])) {
                $this->cache[$keyFormat] = false;
            }
            if (!empty($this->version($key))) {
                $this->cache[$keyFormat] = true;
            }
        }

        return $this->cache[$keyFormat];
    }
